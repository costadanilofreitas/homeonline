//controlador de produto - fábrica
...
angular.module('homeonlineApp')
  ...
  .controller('ProductViewCtrl', function ($scope, $state, $stateParams, Product){
    $scope.product = Product.get({id: $stateParams.id}); //$stateParams é um objeto que contém todas as váriáveis da URL(por exemplo, product id)

    $scope.deleteProduct = function(){
      Product.delete({id: $scope.product._id},
       function success(/*value, responseHeaders */){
       $state.go('products'); //$state permite redirecionamento para um estado ou rota diferente
      }, errorHandler($scope));
    };
  })

  .controller('ProductNewCtrl', function ($scope, $state, Product){
    $scope.product = {}; //cria uma nova instancia
